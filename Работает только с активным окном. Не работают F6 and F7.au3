...
If $p[0][0] > 1 Then
    ...
    Exit
EndIf
#EndRegion StartCheck

;~ $button_About = TrayCreateItem("Инструкция")
;~ $button_Setup = TrayCreateItem("Настройки")
While 1
	If WinWaitActive("Графическое сопоставление") Then ;~ Запуск по нажатию F6
		Do
	...
	ClipPut ( $temp_Done )
	Sleep(100)
	Until WinWaitNotActive("Графическое сопоставление");~ Стоп по нажатию F7
EndIf
WEnd
;~ _IsPressed(76) <> 0